...
def client(region_name):
    global emr
    emr = boto3.client('emr', region_name=region_name, aws_access_key_id=KEY,
                        aws_secret_access_key=SECRET)


def get_security_group_id(group_name, region_name):
    ec2 = boto3.client('ec2', region_name=region_name, aws_access_key_id=KEY,
                        aws_secret_access_key=SECRET)
    response = ec2.describe_security_groups(GroupNames=[group_name])
    return response['SecurityGroups'][0]['GroupId']


def create_cluster(region_name, cluster_name='Airflow-' + str(datetime.now()), release_label='emr-5.22.0',master_instance_type='m5.xlarge', num_core_nodes=1, core_node_instance_type='m5.xlarge'):
...
# Python(kind=pyspark), R(kind=sparkr) and SQL(kind=sql) spark sessions can 
# also be created by changing the value of kind.
